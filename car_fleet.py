...
# at the same position and same speed.
...
# The car starting at 0 does not catch up to any other car,
# so it is a fleet by itself.
# The cars starting at 5 (speed 1) and 3 (speed 3) become a fleet,
# meeting each other at 6. The fleet moves at speed 1
...
def car_fleet(target, position, speed):
    ...
    for p, s in sorted(pair)[::-1]:  # reverse sorted order
        stack.append((target - p) / s)  # appending to the stack est time of each car
        if len(stack) >= 2 and stack[-1] < stack[-2]:  # checking if 2 cars collide
            stack.pop()
    return len(stack)
